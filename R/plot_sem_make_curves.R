if(FALSE){
  ...
  p <- ggplot(NULL) +
    #geom_point() +
    geom_rect(data = df_nodes[df_nodes$shape == "rect", ], aes(xmin = node_xmin, xmax = node_xmax, ymin = node_ymin, ymax = node_ymax), fill = "white", colour = "black") +
    geom_ellipse(data = df_nodes[df_nodes$shape == "oval", ], aes(x0 = x, y0 = y,
                                                                  a = .5*ellipses_a,
                                                                  b = .5*ellipses_b, angle = 0), fill = "white", colour = "black") +
    geom_text(data = df_nodes, aes(x = x, y = y, label = label), size = text_size)+
  ...
  browser()
  p + geom_segment(data = df_cor, aes(x = seg_xmin,
                                      xend = seg_xmax,
                                      y = seg_ymin,
                                      yend = seg_ymax)) +
    geom_curve(data = df_cor, aes(x = edge_xmin, y = edge_ymin, xend = seg_xmin, yend = seg_ymin))
